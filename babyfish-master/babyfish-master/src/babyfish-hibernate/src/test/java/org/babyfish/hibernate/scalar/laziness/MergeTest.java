...
public class MergeTest extends AbstractTest {
    ...
    private static final int OM_IMAGE_ID =
            Metadatas.of(Department.class).getDeclaredScalarProperty("omImage").getId();
    ...
    public void testMergeManagedDepartmentWithoutImage() {
        try (XEntityManager em = this.entityManagerFactory.createEntityManager()) {
            ...
            mergeDepartment(em, department, NEW_DEPARTMENT_NAME);
        }
        try (XEntityManager em = this.entityManagerFactory.createEntityManager()) {
    ...
    public void testMergeDetached2DepartmentWithoutImage() {
        {
            ...
            try (XEntityManager em = this.entityManagerFactory.createEntityManager()) {
                ...
                ((ObjectModel)Department.om(department)).disable(OM_IMAGE_ID);
            }
            try (XEntityManager em = this.entityManagerFactory.createEntityManager()) {
    ...
    public void mergeDetachDepartmentWithImageOnly() {
        {
            ...
            department.setName(NEW_DEPARTMENT_NAME);
            department.setImage(NEW_DEPARTMENT_IMAGE);
            ObjectModelFactory<?> objectModelFactory = Metadatas.of(Department.class).getFactory();
            JPAObjectModelMetadata objectModelMetadata = objectModelFactory.getObjectModelMetadata();
            int nameId = objectModelMetadata.getMappingSources().get("name").getId();
            ((ObjectModel)objectModelFactory.get(department)).disable(nameId);
            try (XEntityManager em = this.entityManagerFactory.createEntityManager()) {
                mergeDepartment(em, department, null);
            }
        }
        try (XEntityManager em = this.entityManagerFactory.createEntityManager()) {
            getDepartment(em, NEW_DEPARTMENT_NAME, false);
    ...
    private static void mergeDepartment(XEntityManager em, Department department, String newName) {
        mergeDepartment(em, department, newName, null);
    }
