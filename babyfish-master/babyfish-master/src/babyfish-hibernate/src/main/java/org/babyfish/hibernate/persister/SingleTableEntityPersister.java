...
public class SingleTableEntityPersister extends org.hibernate.persister.entity.SingleTableEntityPersister {
    ...
    private class BridgeImpl implements EntityPersisterBridge {
        ...
        public EntityPersister getEntityPersister() {
            return SingleTableEntityPersister.this;
        }

        @Override
        public int getTableSpan() {
            return SingleTableEntityPersister.this.getTableSpan();
        }

        @Override
        public String getTableName(int tableIndex) {
            return SingleTableEntityPersister.this.getTableName(tableIndex);
        }

        @Override
        public boolean isPropertyOfTable(int propertyIndex, int tableIndex) {
            return SingleTableEntityPersister.this.isPropertyOfTable(propertyIndex, tableIndex);
        }

        @Override
        public boolean[] getTableUpdateNeeded(int[] updatePropertyIndexes) {
            return SingleTableEntityPersister.this.getTableUpdateNeeded(updatePropertyIndexes, false);
        }

        @Override
        public String getRowIdName() {
            return SingleTableEntityPersister.this.rowIdName;
        }

        @Override
        public String[] getKeyColumns(int tableIndex) {
            return SingleTableEntityPersister.this.getKeyColumns(tableIndex);
        }

        @Override
        public String[] getPropertyColumnNames(int propertyIndex) {
            return SingleTableEntityPersister.this.getPropertyColumnNames(propertyIndex);
        }

        @Override
        public boolean[][] getPropertyColumnUpdateable() {
            return SingleTableEntityPersister.this.getPropertyColumnUpdateable();
        }
    }
}
