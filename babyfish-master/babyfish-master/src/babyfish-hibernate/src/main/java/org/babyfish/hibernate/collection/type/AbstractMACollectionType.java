...
public abstract class AbstractMACollectionType implements UserCollectionType, ParameterizedType {
    ...
    private JPAAssociationProperty jpaAssociationProperty;
    ...
    public final void setParameterValues(MACollectionProperties parameters) {
        ...
        try { 
            if (this.jpaAssociationProperty != null) {
                throw new IllegalStateException(LAZY_RESOURCE.get().parameterValuesHaveBeenSet());
            }
            this.jpaAssociationProperty = parameters.getJPAAssociationProperty();
        } finally {
            lock.unlock();
        }
    }
    
    protected final JPAAssociationProperty getJPAAssociationProperty() {
        Lock lock;
        ...
        try {
            ...
            if (jpaAssociationProperty == null) {
                throw new IllegalStateException(LAZY_RESOURCE.get().parameterValuesHaveNotBeenSet());
            }
    ...
    protected final String getRole() {
        ...
        JPAAssociationProperty property = this.jpaAssociationProperty;
        if (role == null && property != null) {
            this.role = role = 
                    property.getDeclaringObjectModelMetadata().getOwnerClass().getName() +
                    '.' +
                    property.getOwnerProperty().getName();
        }
        return role;
    }
    ...
    private interface Resource {
        ...
        String parameterValuesHaveBeenSet();
        
        String parameterValuesHaveNotBeenSet();
    }
}
