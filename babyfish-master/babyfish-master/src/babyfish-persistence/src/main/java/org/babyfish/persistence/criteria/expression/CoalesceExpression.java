...
public class CoalesceExpression<T> extends AbstractExpression<T> implements Coalesce<T> {
    ...
    public Class<? extends T> getJavaType() {
        if (this.javaType == null) {
            throw new IllegalStateException(LAZY_RESOURCE.get().isEmpty(CoalesceExpression.class));
        }
        ...
        if (value != null) {
            if (this.values.isEmpty()) {
                mustHaveExplicitDataTypeWhen(
                        LAZY_RESOURCE.get().whenCoalesceIsEmpty(CoalesceExpression.class),
                        "value", 
                        value
                );
            }
    ...
    private interface Resource {
        ...
        String whenCoalesceIsEmpty(Class<CoalesceExpression> coalesceExpressionType);
        
        @SuppressWarnings("rawtypes")
        String isEmpty(Class<CoalesceExpression> coalesceExpressionType);
    }
}
