...
public class InPredicate<T> extends AbstractSimplePredicate implements In<T> {
    ...
    public List<Partition<T>> getPartitions() {
        ...
        if (partitions == null) {
            throw new IllegalStateException(
                    LAZY_RESOURCE.get().methodCanOnlyBeInvokedWhenThisIsFrozen("getPartitions", InPredicate.class)
            );
        }
        return partitions;
    }
    ...
    private interface Resource {
        ...
        String methodCanOnlyBeInvokedWhenThisIsFrozen(String methodName, Class<InPredicate> predicateType);
    }
}
