...
public class CriteriaUpdateImpl<T> extends AbstractNode implements XCriteriaUpdate<T> {
    ...
    private void rootMustBeNull() {
        if (this.root != null) {
            throw new IllegalStateException(LAZY_RESOURCE.get().rootMustBeNull(this.getClass()));
        }
    }

    private void rootMustNotBeNull() {
        if (this.root == null) {
            throw new IllegalStateException(LAZY_RESOURCE.get().rootMustNotBeNull(this.getClass()));
        }
    }
    ...
    private void pathMustBeChildOfRoot(Path<?> attribute) {
        ...
        throw new IllegalArgumentException(LAZY_RESOURCE.get().pathMustBeChildOfRoot(this.getClass()));
    }
    
    private interface Resource {
        
        String rootMustBeNull(Class<?> thisType);
        
        String rootMustNotBeNull(Class<?> thisType);
        
        String pathMustBeChildOfRoot(Class<?> thisType);
    }
}
