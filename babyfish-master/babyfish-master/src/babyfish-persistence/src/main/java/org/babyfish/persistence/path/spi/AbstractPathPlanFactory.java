...
import org.babyfish.persistence.path.spi.PathPlanKey.SubKey;
...
public abstract class AbstractPathPlanFactory {
        ...
        private JoinNodeImpl(
            ...
            if (name.equals(parentEntityDelegate.getIdPropertyName())) {
                throw new IllegalArgumentException(
                        LAZY_RESOURCE.get().joinPropertyMustBeAssociationProperty(parentNode.getPath(), name)
                );
            }
        ...
        void addFetchPath(FetchPath fetchPath) {
            ...
            while (node != null) {
                ...
                if (node.getName().equals(this.getIdPropertyName())) {
                    throw new IllegalArgumentException(
                            LAZY_RESOURCE.get().fetchPathCanNotContainId(fetchPath, node.getName())
                    );
                }
                ...
                if (propertyDelegate.isAssociation()) {
                    ...
                    if (nextNode != null) {
                        throw new IllegalArgumentException(
                                LAZY_RESOURCE.get().fetchPathConOnlyContainOneScalar(fetchPath, node.getName())
                        );
                    }
                    if (node.getGetterType() == GetterType.REQUIRED) {
                        throw new IllegalArgumentException(
                                LAZY_RESOURCE.get().scalarPropertyCanNotBeRequired(fetchPath, node.getName())
                        );
                    }
                    if (node.getCollectionFetchType() == CollectionFetchType.PARTIAL) {
                        throw new IllegalArgumentException(
                                LAZY_RESOURCE.get().scalarPropertyCanNotBePartial(fetchPath, node.getName())
                        );
                    }
                    ...
                    if (propertyDelegate != null && propertyDelegate.isAssociation()) {
                        ...
                        for (; node != null; node = node.getNextNode()) {
                            ...
                            if (node.getGetterType() == GetterType.REQUIRED) {
                                ...
                                throw new IllegalArgumentException(
                                        LAZY_RESOURCE.get().requiredOrderPropertyMustBeAssociationProperty(
            ...
            if (orderNodes == null) {
                ...
                if (existingOrderNode != null) {
                    if (existingOrderNode.post != orderNodeImpl.post) {
                        throw new IllegalArgumentException(
                                LAZY_RESOURCE.get().simpleOrderPathStateAreConflict(
        ...
        public String getPath() {
            ...
            if (path == null) {
                ...
                if (!Nulls.isNullOrEmpty(this.loadedScalarNames)) {
                    builder.append('[');
                    Joins.join(this.loadedScalarNames, builder);
                    builder.append(']');
                }
    ...
    private interface Resource {
        ...
        String methodCanNotReturnNull(Class<?> ownerType, String methodSignature);
        
        String joinPropertyMustBeAssociationProperty(String parentPath, String name);
        
        String fetchPathCanNotContainId(FetchPath fetchPath, String idPropertyName);
        
        String fetchPathConOnlyContainOneScalar(FetchPath fetchPath, String firstScalarNodeName);
        
        String scalarPropertyCanNotBeRequired(FetchPath fetchPath, String scalarNodeName);
        
        String scalarPropertyCanNotBePartial(FetchPath fetchPath, String scalarNodeName);
        
        String requiredOrderPropertyMustBeAssociationProperty(
                String parentPath,
                GetterType getterType,
                String name);

        String simpleOrderPathStateAreConflict(
                String parentPath1, 
                String state1,
                String parentPath2, 
                String state2);
