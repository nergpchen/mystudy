...
public abstract class AbstractNode implements Serializable {
        ...
        if (argumentValue != null) {
            ...
            if (criteriaBuilder != ((AbstractNode)argumentValue).criteriaBuilder) {
                throw new IllegalArgumentException(
                        LAZY_RESOURCE.get().childNodeMustUnderTheSameCriteriaBuilder(
                                argumentName,
    ...
    protected final void checkState() {
        if (this.frozenBy != null) {
            throw new IllegalStateException(LAZY_RESOURCE.get().theCurrentNodeIsFrozen(QueryContext.class));
        }
    }
    ...
    final void freeze(QueryContext queryContext) {
        ...
        if (this.frozenBy != queryContext) {
            if (this.frozenBy != null) {
                throw new IllegalStateException(LAZY_RESOURCE.get().freezeWhenCurrentNodeIsFrozenByOther(QueryContext.class));
            }
    ...
    final void unfreeze(QueryContext queryContext) {
        Arguments.mustNotBeNull("queryContext", queryContext);
        if (this.frozenBy != queryContext) {
            throw new IllegalStateException(LAZY_RESOURCE.get().unfreezeWhenCurrentNodeIsFrozenByOther(QueryContext.class));
        }
        this.onUnfrozen();
    ...
    private interface Resource {
        ...
        String childNodeMustUnderTheSameCriteriaBuilder(
                String argumentName, 
                Class<XCriteriaBuilder> xCriteriaBuilderType);
        
        String theCurrentNodeIsFrozen(Class<QueryContext> queryContextType);

        String freezeWhenCurrentNodeIsFrozenByOther(Class<QueryContext> queryContextType);

        String unfreezeWhenCurrentNodeIsFrozenByOther(Class<QueryContext> queryContextType);
    }
