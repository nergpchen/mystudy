...
public class QueryContext implements AutoCloseable {
    ...
    public PathId.Allocator secondaryPathIdAllocator() {
        if (this.pathIdAllocator == null) {
            throw new IllegalStateException(LAZY_RESOURCE.get().queryContextIsClosed(QueryContext.class));
        }
        ...
        public void visitParameterExpression(ParameterExpression<?> parameterExpression) {
            if (parameterExpression.getName() == null) {
                ...
                if (!parameterExpressionImpl.setPosition(++this.parameterPositionSequence)) {
                    throw new IllegalProgramException(LAZY_RESOURCE.get().unnamedParameterCanNotBeUsedTwice());
                }
        ...
        private List<Entity> getEntities(XCommonAbstractCriteria commonAbstractCriteria) {
            ...
            if (entities == null) {
                if (commonAbstractCriteria instanceof XCriteriaUpdate<?>) {
                    ...
                    if (root == null) {
                        throw new IllegalStateException(LAZY_RESOURCE.get().noRoots(XCriteriaUpdate.class));    
                    }
                    ...
                    if (roots.isEmpty()) {
                        throw new IllegalStateException(LAZY_RESOURCE.get().noRoots(XAbstractQuery.class));
                    }
...
private static class EntityImpl implements Entity {
        ...
        public String getRenderAlias() {
            ...
            int id = this.id;
            if (id == -1) {
                throw new IllegalStateException(LAZY_RESOURCE.get().entityIdHasNotBeenAllocated(Entity.class));
            }
    ...
    private interface Resource {
        ...
        String queryContextIsClosed(Class<QueryContext> queryContextType);
        
        String unnamedParameterCanNotBeUsedTwice();
        
        String noRoots(Class<? extends XCommonAbstractCriteria> xAbstractQueryType);
        
        String entityIdHasNotBeenAllocated(Class<Entity> entityType);
    }
}
