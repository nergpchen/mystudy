...
public class HashEntries<K, V> extends AbstractRootBaseEntriesImpl<K, V> implements BaseEntries<K, V> {
    ...
    protected V putWithoutTriggerFlushing(
        ...
        if (be != null) {
            ...
            if (trigger != null) {
                if (isKeyStrict) {
                    trigger.preChange(be, key, value);
                } else {
                    trigger.preChange(be, be.getKey(), value);
                }
            }
        ...
        if (key == null) {
            this.addEntry(0, null, value, 0, attachProcessor);
        } else {
            int hash = hash(this.keyUnifiedComparator().hashCode(key));
            int bucketIndex = (this.buckets.length - 1) & hash;
            this.addEntry(hash, key, value, bucketIndex, attachProcessor);
        }
        this.scale(true);
        return null;
    }
    ...
    protected final void putAllWithoutTriggerFlushing(
            Map<? extends K, ? extends V> m, AttachProcessor<K, V> attachProcessor) {
        ...
        if (trigger == null) {
            if (attachProcessor.beginExcute()) {
                try {
                    for (Entry<? extends K, ? extends V> e : m.entrySet()) {
                        ...
                        if (be != null) {
                            if (isKeyStrict) {
                                be.setRawKey(key, keyEqualityComparator);
                            }
                            be.value = e.getValue();
                            be.recordChange();
                        } else {
            ...
            for (Entry<? extends K, ? extends V> e : m.entrySet()) {
                ...
                BaseEntryImpl<K, V> be = this.getBaseEntry(key);
                if (be != null) {
                    trigger.preChange(be, isKeyStrict ? key : be.getKey(), value);
                } else {
                ...
                try {
                    for (int i = 0; i < mSize; i++) {
                        ...
                        if (be != null) {
                            ...
                            if (key == null) {
                                this.addEntry(0, null, value, 0, AbstractBaseEntriesImpl.<K, V>emptyAttachProcessor());
                            } else {
                                int hash = hash(keyUnifiedComparator.hashCode(key));
                                int bucketIndex = (this.buckets.length - 1) & hash;
                                this.addEntry(hash, key, value, bucketIndex, AbstractBaseEntriesImpl.<K, V>emptyAttachProcessor());
                            }
                        }
                    }
                    this.scale(true);
        ...
        if (trigger == null) {
            for (K key : c) {
                ...
                if (be != null) {
                    ...
                    if (key == null) {
                        this.addEntry(0, null, (V)PRESENT, 0, AbstractBaseEntriesImpl.<K, V>emptyAttachProcessor());
                    } else {
                        int hash = hash(keyUnifiedComparator.hashCode(key));
                        this.addEntry(hash, key, (V)PRESENT, (this.buckets.length - 1) & hash, AbstractBaseEntriesImpl.<K, V>emptyAttachProcessor());
                    }
                }
            }
        } else {
                    ...
                    for (int i = 0; i < cSize; i++) {
                        ...
                        if (be != null) {
                            ...
                            if (key == null) {
                                ...
                                int hash = hash(keyUnifiedComparator.hashCode(key));
                                int bucketIndex = (this.buckets.length - 1) & hash;
                                this.addEntry(hash, key, (V)PRESENT, bucketIndex, AbstractBaseEntriesImpl.<K, V>emptyAttachProcessor());
                            }
                        }
                    }
    ...
    protected void deleteBaseEntryImpl(BaseEntryImpl<K, V> e) {
        
        this.modCount++;
        BaseEntryImpl<K, V>[] buckets = this.buckets;
        int bucketIndex = (buckets.length - 1) & e.hash;
