...
public final class LinkedElements<E> extends AbstractBaseElementsImpl<E> {
    ...
    public LinkedElements(
        ...
        super(comparatorOrEqualityComparatorOrUnifiedComparator);
    ...
    @Override
    public int indexOf(int subListHeadHide, int subListTailHide, Object o, ElementMatcher<? super E> matcher) {
        ...
        if (lastAccess !=null && 
                lastAccess.matchFirstWithoutOtherMatcher && 
        ...
        if (o == null) {
            ...
            while (absIndex < fenceIndex) {
                if (unifiedComparator.equals((E)o, node.value)) {
                    if (o == null || matcher == null || matcher.match((E)o, node.value)) {
                        break;
                    }
                }
                absIndex++;
        ...
        if (absIndex != fenceIndex) {
            if (this.readerOptimizationType == ReaderOptimizationType.OPTIMIZE_READING) {
                boolean fullScanWithoutOtherMatcher = 
                    matcher == null && subListHeadHide == 0 && subListTailHide == 0;
                if (lastAccess == null) {
                    this.lastAccess = new LastAccess<E>(absIndex, node, fullScanWithoutOtherMatcher, false);
                } else {
                    lastAccess.index = absIndex;
                    lastAccess.node = node;
                    lastAccess.matchFirstWithoutOtherMatcher = fullScanWithoutOtherMatcher;
                }
            }
    ...
    @SuppressWarnings("unchecked")
    @Override
    public int lastIndexOf(int subListHeadHide, int subListTailHide, Object o, ElementMatcher<? super E> matcher) {
        ...
        LastAccess<E> lastAccess = this.lastAccess;
        if (lastAccess !=null && 
                lastAccess.matchLastWithoutOtherMatcher && 
                lastAccess.index >= subListHeadHide && 
                lastAccess.index < this.allSize - subListTailHide) {
            if (o == null ? 
                    lastAccess.node.value == null : 
        ...
        if (absIndex != subListHeadHide - 1) {
            if (this.readerOptimizationType == ReaderOptimizationType.OPTIMIZE_READING) {
                boolean fullScanWithoutOtherMatcher = 
                    matcher == null && subListHeadHide == 0 && subListTailHide == 0;
                if (lastAccess == null) {
                    this.lastAccess = new LastAccess<E>(absIndex, node, false, fullScanWithoutOtherMatcher);
                } else {
                    lastAccess.index = absIndex;
                    lastAccess.node = node;
                    lastAccess.matchLastWithoutOtherMatcher = fullScanWithoutOtherMatcher;
                }
            }
            return absIndex - subListHeadHide;
        }
            ...
            try {
                ...
                if (this.readerOptimizationType == ReaderOptimizationType.OPTIMIZE_READING && lastAccess != null) {
                    if (absIndex <= lastAccess.index) {
                        if (lastAccess.matchFirstWithoutOtherMatcher &&
                                (element == null ? lastAccess.node.value == null : element.equals(lastAccess.node.value))) {
                            lastAccess.matchFirstWithoutOtherMatcher = false;
                        }
                        lastAccess.index++;
                    } else {
                        if (lastAccess.matchLastWithoutOtherMatcher && 
                                (element == null ? lastAccess.node.value == null : element.equals(lastAccess.node.value))) {
                            lastAccess.matchLastWithoutOtherMatcher = false;
                        }
                    }
                }
    ...
    protected E setImpl(E element, AttachProcessor<E> attachProcessor) {
        NodeImpl<E> node = this.nodeAt(attachProcessor.getExpectedIndex(true), true);
        Trigger<E> trigger = attachProcessor.getTrigger();
        if (trigger != null) {
            trigger.preChange(
    ...
    protected boolean remove(int subListHeadHide, int subListTailHide, Object o, Trigger<E> trigger) {
        ...
        int index = this.indexOf(subListHeadHide, subListTailHide, o, null);
        if (index != -1) {
    ...
    private static class LastAccess<E> {
        ...
        boolean matchFirstWithoutOtherMatcher;
        
        boolean matchLastWithoutOtherMatcher;
        ...
        LastAccess(
            ...
            this.index = index;
            this.node = node;
            this.matchFirstWithoutOtherMatcher = matchFirstWithoutOtherMatcher;
            this.matchLastWithoutOtherMatcher = matchLastWithoutOtherMatcher;
        }
    }
