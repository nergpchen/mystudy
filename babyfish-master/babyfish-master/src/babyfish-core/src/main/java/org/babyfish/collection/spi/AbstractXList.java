...
public abstract class AbstractXList<E> extends StatefulObject implements XList<E> {
    ...
    public boolean isNonFairLockSupported() {
        return this.baseElements.isNonFairLockSupported();
    }
    ...
    public void addConflictVoter(ListConflictVoter<E> voter) {
        this.baseElements.combineConflictVoter(voter);
    }
    
    @Override
    public void removeConflictVoter(ListConflictVoter<E> voter) {
        this.baseElements.removeConflictVoter(voter);
    }
    ...
    public int indexOf(Object o, ElementMatcher<? super E> matcher) {
        return this.baseElements.indexOf(this.headHide(), this.tailHide(), o, matcher);
    }

    @Override
    public int lastIndexOf(Object o, ElementMatcher<? super E> matcher) {
        return this.baseElements.lastIndexOf(this.headHide(), this.tailHide(), o, matcher);
    }

    @Override
    public boolean contains(Object o, ElementMatcher<? super E> matcher) {
        return this.baseElements.indexOf(this.headHide(), this.tailHide(), o, matcher) != -1;
    }

    @Override
    public boolean containsAll(Collection<?> c, ElementMatcher<? super E> matcher) {
        for (Object o : c) {
            if (this.baseElements.indexOf(this.headHide(), this.tailHide(), o, matcher) == -1) {
                return false;
            }
        }
        return true;
    ...
    public XListIterator<E> listIterator(final int index) {
        ...
        final boolean nonFairLockSupported = this.baseElements.isNonFairLockSupported();
    ...
    protected static class SubListImpl<E> extends AbstractXList<E> implements XListView<E> {
        ...
        private void checkConcurrentModification() {
            if (this.expectedModCount != this.baseElements.modCount()) {
                throw new ConcurrentModificationException(LAZY_RESOURCE.get().viewBecameInvalid(this.getClass()));
            }
        }
    ...
    private interface Resource {
        ...
        String viewBecameInvalid(Class<?> thisType);
    }
}
