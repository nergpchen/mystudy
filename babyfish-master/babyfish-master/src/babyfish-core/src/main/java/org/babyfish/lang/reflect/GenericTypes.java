...
public class GenericTypes {
        ...
        if (!ClassInfo.isGenericClass(rawType)) {
            if (hasActualTypeArguments) {
                Arguments.mustBeEmptyWhen(
                        LAZY_RESOURCE.get().whenTheTypeIsNotGeneric("rawType"), 
                        "actualTypeArguments", 
                        actualTypeArguments);
            }
            return rawType;
            ...
            if (hasActualTypeArguments) {
                ...
                if (typeParameterCount > actualTypeArguments.length) {
                    throw new IllegalArgumentException(
                            LAZY_RESOURCE.get().invalidTypeArgumentCount(
                                    rawType, 
                                    typeParameterCount, 
    ...
    public static Type makeParameterizedType(
        ...
        if (GenericTypes.eraseGenericType(ownerType) != rawType.getDeclaringClass()) {
            throw new IllegalArgumentException(
                    LAZY_RESOURCE.get().erasedOwnerTypeMustBeDeclaringOfRawType(
                            "ownerType",
                            ownerType,
                            "rawType",
                            rawType)
        ...
        if (!(ownerType instanceof ParameterizedTypeImpl) && rawType.getTypeParameters().length == 0) {
            if (actualTypeArguments != null && actualTypeArguments.length != 0) {
                Arguments.mustBeEmptyWhen(
                        LAZY_RESOURCE.get().whenOwnerTypeIsClassWithoutTypeParameter("ownerType"), 
                        "actualTypeArguments", 
                        actualTypeArguments
                );
            }
        ...
        public ParameterizedTypeImpl(
            ...
            if (!(ownerType instanceof ParameterizedType)) {
                Arguments.mustNotBeEmptyWhen(
                        LAZY_RESOURCE.get().whenOwnerTypeIsNotParameterizedType("ownerType"),
            ...
            for (int i = 0; i < typeVariables.length; i++) {
                ...
                for (Type bound : typeVariables[i].getBounds()) {
                    if (!GenericTypes.privateEraseGenericType(bound, true).isAssignableFrom(erasedActualTypeArgument)) {
                        throw new IllegalArgumentException(
                                LAZY_RESOURCE.get().invalidTypeArgument(
                                        this.rawType,
                                        i,
                                        bound, 
                                        actualTypeArgument
    ...
    private interface Resource {
        ...
        String invalidTypeArgumentCount(Class<?> rawType, int expectCount, int actualActualType);
        
        String invalidTypeArgument(Class<?> rawType, int typeParameterIndex, Type bound, Type actualTypeArgument);

        String erasedOwnerTypeMustBeDeclaringOfRawType(
                String ownerTypeParameterName,
                Type ownerType, 
                String rawTypeParameterName, 
                Class<?> rawType);
        
        String whenTheTypeIsNotGeneric(String parameterName);
        
        String whenOwnerTypeIsClassWithoutTypeParameter(String parameterName);
        
        String whenOwnerTypeIsNotParameterizedType(String ownerTypeParameterName);
    }
}
