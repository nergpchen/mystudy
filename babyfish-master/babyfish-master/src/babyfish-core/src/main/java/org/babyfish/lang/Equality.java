...
public class Equality<T> {
    ...
    public boolean returnValue() {
        if (this.returnValue == 0) {
            throw new IllegalStateException(
                    LAZY_RESOURCE.get().illegalStateToGetReturnValue(Equality.class)
            );
        }
    ...
    private static void validateEqualsMethod(Method method) {
        if (
            ...
            throw new IllegalProgramException(
                    LAZY_RESOURCE.get().invalidOverrideEqualityMethod(method, OverrideEquality.class)
            );
        }
    }
    ...
    private interface Resource {
        ...
        String invalidOverrideEqualityMethod(
                Method method,
                Class<OverrideEquality> overrideEqualtiyTypeConstant);
        
        @SuppressWarnings("rawtypes")
        String illegalStateToGetReturnValue(
