...
public abstract class ClassWrapper {
    ...
    private BinaryFunc<Object, Object[], Object> factory;
    ...
    protected ClassWrapper(Class<?> rawType, Class<?> ... argumentTypes) {
        ...
        if (this.rawInterfaceTypes == null) {
            throw new IllegalProgramException(
                    LAZY_RESOURCE.get().eachElementOfReturnValueCanOnlyBeInterface(
                            this.getClass(),
        ...
        if (this.interfaceTypes.isEmpty()) {
            throw new IllegalProgramException(LAZY_RESOURCE.get().noInterfaces(this.getClass()));
        }
        ...
        Class<BinaryFunc<Object, Object[], Object>> factoryClass = (Class)this.createFactoryClass();
        try {
    ...
    private static ClassWrapper getInstance0(Class<? extends ClassWrapper> wrapperClass) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    LAZY_RESOURCE.get().mustSupportDefaultConstructor(wrapperClass)
            );
        }
    ...
    protected final Method generateInvokeRawMethodCode(XMethodVisitor mv, MethodImplementation methodImplementation) {
        ...
        if (rawMethod == null) {
            throw new IllegalProgramException(
                    LAZY_RESOURCE.get().canNotInvokeRawMethod(
                            this.getClass(),
    ...
    private Class<?> createFactoryClass() {
        ...
        try {
            ...
            throw new IllegalProgramException(
                    LAZY_RESOURCE.get().resultClassMustSupportDefaultConstructor(
                            this.getClass(),
                            this.resultClass
                    )
            );
        ...
        if (!Modifier.isPublic(constructor.getModifiers())) {
            throw new IllegalProgramException(
                    LAZY_RESOURCE.get().defaultConstructorOfResultClassMustBePublic(
                            this.getClass(),
                            this.resultClass
                    )
            );
        ...
        final String resultFactoryClassName =
            ...
            "}";
        return ASM.loadDynamicClass(
                BinaryFunc.class.getClassLoader(), 
                resultFactoryClassName, 
                BinaryFunc.class.getProtectionDomain(), 
                new Action<ClassVisitor>() {
                    @Override
                    public void run(ClassVisitor cv) {
    ...
    private void generateFactoryClass(ClassVisitor cv, String factoryClassname) {
        cv.visit(
                ...
                null, 
                "java/lang/Object", 
                new String[] { ASM.getInternalName(BinaryFunc.class) });
    ...
    private interface Resource {
        ...
        String mustSupportDefaultConstructor(Class<? extends ClassWrapper> runtimeType);

        String eachElementOfReturnValueCanOnlyBeInterface(
                Class<? extends ClassWrapper> runtimeType, String methodName);

        String noInterfaces(Class<? extends ClassWrapper> runtimeType);

        String canNotInvokeRawMethod(
                Class<? extends ClassWrapper> runtimeType,
                Set<MethodDescriptor> selfAndBridgeDescriptors,
                List<Class<?>> rawInterfaceTypes);

        String resultClassMustSupportDefaultConstructor(
        Class<? extends ClassWrapper> runtimeType, Class<?> resultClass);

        String defaultConstructorOfResultClassMustBePublic(
                Class<? extends ClassWrapper> runtimeType, 
                Class<?> resultClass);
