/*
 * BabyFish, Object Model Framework for Java and JPA.
 * https://github.com/babyfish-ct/babyfish
 *
 * Copyright (c) 2008-2015, Tao Chen
 *
 * This copyrighted material is made available to anyone wishing to use, modify,
 * copy, or redistribute it subject to the terms and conditions of the GNU
 * Lesser General Public License, as published by the Free Software Foundation.
 *
 * Please visit "http://opensource.org/licenses/LGPL-3.0" to know more.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY
 * or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU Lesser General Public License
 * for more details.
 */
package org.babyfish.model.spi;

import java.io.ObjectStreamException;
import java.io.Serializable;

import org.babyfish.lang.Arguments;

/**
 * @author Tao Chen
 */
public final class ObjectModelFactoryWritingReplacement implements Serializable {
    
    private static final long serialVersionUID = -6707391612302038809L;

    private ObjectModelFactoryProvider objectModelFactoryProvider;
    
    private Class<?> objectModelClass;
    
    public ObjectModelFactoryWritingReplacement(
            ObjectModelFactoryProvider objectModelFactoryProvider,
            Class<?> objectModelClass) {
        this.objectModelFactoryProvider = 
                Arguments.mustNotBeNull(
                        "objectModelFactoryProvider", 
                        objectModelFactoryProvider);
        this.objectModelClass = 
                Arguments.mustNotBeNull(
                        "objectModelClass", 
                        objectModelClass);
    }

    protected final Object readResolve() throws ObjectStreamException {
        return this.objectModelFactoryProvider.getFactory(this.objectModelClass);
    }
}
