...
import org.babyfish.reference.event.IndexedValueListener;
import org.babyfish.reference.event.ValueEvent;
import org.babyfish.reference.event.modification.IndexedReferenceModifications;
...
public class MAIndexedReferenceImpl<T> extends MAReferenceImpl<T> implements MAIndexedReference<T> {
    ...
    private static final Combiner<IndexedValueListener> INDEXED_VALUE_LISTENER_COMBINER = 
        (Combiner)Combiners.of(IndexedValueListener.class);
    ...
    public void addIndexedValueListener(IndexedValueListener<? super T> listener) {
        this.indexedValueListener = 
            (IndexedValueListener<T>)INDEXED_VALUE_LISTENER_COMBINER.combine(
                    (IndexedValueListener)this.indexedValueListener, 
                    (IndexedValueListener)listener);
    }
    ...
    public void removeIndexedValueListener(IndexedValueListener<? super T> listener) {
        this.indexedValueListener = 
            (IndexedValueListener<T>)INDEXED_VALUE_LISTENER_COMBINER.remove(
                    (IndexedValueListener)this.indexedValueListener, 
                    (IndexedValueListener)listener);
    }
    ...
    public int setIndex(final int index) {
        ...
        this.setRaw(event, new EmptyAction() {
            @Override
            public void run() {
                MAIndexedReferenceImpl.this.index = index;
            }
        });
        return oldIndex;
    }
    ...
    public T set(final T value) {
        ...
        this.setRaw(event, new EmptyAction() {
            @Override
            public void run() {
                MAIndexedReferenceImpl.this.value = value;
            }
        });
        return oldValue;
    }
