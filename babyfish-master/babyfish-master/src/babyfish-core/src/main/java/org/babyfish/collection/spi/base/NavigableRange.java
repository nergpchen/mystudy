...
public class NavigableRange<E> implements Serializable {
    ...
    private NavigableRange(
        ...
        if (hasFrom && hasTo && this.compare(from, to) > 0) {
            throw new IllegalArgumentException(LAZY_RESOURCE.get().formGreaterThanTo(from, to));
        } 
        if (hasFrom) {
    ...
    public static <E> NavigableRange<E> subRange(
            ...
            boolean hasTo, E to, boolean toInclusive) {
        if (!hasFrom && !hasTo) {
            throw new IllegalArgumentException(LAZY_RESOURCE.get().parametersCanNotBeFalseSimultaneously("hasFrom", "hasTo"));
        }
        if (Arguments.mustNotBeNull("parentEntries", parentEntries).comparator() == null) {
            if (hasFrom) {
                Arguments.mustBeInstanceOfValueWhen(
                        LAZY_RESOURCE.get().whenExpressionIsNullAndParameterIsTrue("parentEntries.comparator()", "hasFrom"), 
                        "from", 
            ...
            if (hasTo) {
                Arguments.mustBeInstanceOfValueWhen(
                        LAZY_RESOURCE.get().whenExpressionIsNullAndParameterIsTrue("parentEntries.comparator()", "hasTo"),
                        "to", 
    ...
    public NavigableRange(XNavigableSetView<E> view, Func<XNavigableSetView<E>, XNavigableSet<E>> parentLambda) {
    ...
    public NavigableRange(XNavigableMapView<E, ?> view, Func<XNavigableMapView<E, ?>, XNavigableMap<E, ?>> parentLambda) {
        Arguments.mustNotBeNull("view", view);
        Arguments.mustNotBeNull("parentLambda", parentLambda);
        this.fromInclusive = true;
        this.toInclusive = true;
    ...
    @SuppressWarnings("unchecked")
    private void accept(XNavigableSetView<E> view, Func<XNavigableSetView<E>, XNavigableSet<E>> parentLambda) {
        
        XNavigableSet<E> parent = parentLambda.run(view);
        ...
        if (viewInfo instanceof NavigableSetViewInfos.DescendingSet) {
            ...
            if (viewInfo instanceof SortedSetViewInfos.HeadSetByToElement) {
                ...
                if (viewInfo instanceof NavigableSetViewInfos.HeadSetByToElementAndInclusive) {
                    toInclusive = ((NavigableSetViewInfos.HeadSetByToElementAndInclusive)viewInfo).isInclusive();
                } else {
                    toInclusive = DEFAULT_TO_INCLUSIVE;
                }
                E to = (E)((SortedSetViewInfos.HeadSetByToElement)viewInfo).getToElement();
                this.acceptTo(to, toInclusive);
            } else if (viewInfo instanceof SortedSetViewInfos.TailSetByFromElement) {
                boolean fromInclusive;
                if (viewInfo instanceof NavigableSetViewInfos.TailSetByFromElementAndInclusive) {
                    fromInclusive = ((NavigableSetViewInfos.TailSetByFromElementAndInclusive)viewInfo).isInclusive();
                } else {
                    fromInclusive = DEFAULT_FROM_INCLUSIVE;
                }
                E from  = (E)((SortedSetViewInfos.TailSetByFromElement)viewInfo).getFromElement();
                this.acceptFrom(from, fromInclusive);
            } else {
                boolean fromInclusive;
                boolean toInclusive;
                if (viewInfo instanceof NavigableSetViewInfos.SubSetByFromElementAndFromInclusiveAndToElementAndToInclusive) {
                    fromInclusive = ((NavigableSetViewInfos.SubSetByFromElementAndFromInclusiveAndToElementAndToInclusive)viewInfo).isFromInclusive();
                    toInclusive = ((NavigableSetViewInfos.SubSetByFromElementAndFromInclusiveAndToElementAndToInclusive)viewInfo).isToInclusive();
                } else {
                    fromInclusive = DEFAULT_FROM_INCLUSIVE;
                    toInclusive = DEFAULT_TO_INCLUSIVE;
                }
                E from  = (E)((SortedSetViewInfos.SubSetByFromElementAndToElement)viewInfo).getFromElement();
                E to = (E)((SortedSetViewInfos.SubSetByFromElementAndToElement)viewInfo).getToElement();
                this.acceptFrom(from, fromInclusive);
                this.acceptTo(to, toInclusive);
            }
        }
    ...
    @SuppressWarnings("unchecked")
    private void accept(XNavigableMapView<E, ?> view, Func<XNavigableMapView<E, ?>, XNavigableMap<E, ?>> parentLambda) {
        
        XNavigableMap<E, ?> parent = parentLambda.run(view);
        ...
        if (viewInfo instanceof NavigableMapViewInfos.DescendingMap) {
            ...
            if (viewInfo instanceof SortedMapViewInfos.HeadMapByToKey) {
                ...
                if (viewInfo instanceof NavigableMapViewInfos.HeadMapByToKeyAndInclusive) {
                    toInclusive = ((NavigableMapViewInfos.HeadMapByToKeyAndInclusive)viewInfo).isInclusive();
                } else {
                    toInclusive = DEFAULT_TO_INCLUSIVE;
                }
                E to = (E)((SortedMapViewInfos.HeadMapByToKey)viewInfo).getToKey();
                this.acceptTo(to, toInclusive);
            } else if (viewInfo instanceof SortedMapViewInfos.TailMapByFromKey) {
                boolean fromInclusive;
                if (viewInfo instanceof NavigableMapViewInfos.TailMapByFromKeyAndInclusive) {
                    fromInclusive = ((NavigableMapViewInfos.TailMapByFromKeyAndInclusive)viewInfo).isInclusive();
                } else {
                    fromInclusive = DEFAULT_FROM_INCLUSIVE;
                }
                E from  = (E)((SortedMapViewInfos.TailMapByFromKey)viewInfo).getFromKey();
                this.acceptFrom(from, fromInclusive);
            } else {
                boolean fromInclusive;
                boolean toInclusive;
                if (viewInfo instanceof NavigableMapViewInfos.SubMapByFromKeyAndFromInclusiveAndToKeyAndToInclusive) {
                    fromInclusive = ((NavigableMapViewInfos.SubMapByFromKeyAndFromInclusiveAndToKeyAndToInclusive)viewInfo).isFromInclusive();
                    toInclusive = ((NavigableMapViewInfos.SubMapByFromKeyAndFromInclusiveAndToKeyAndToInclusive)viewInfo).isToInclusive();
                } else {
                    fromInclusive = DEFAULT_FROM_INCLUSIVE;
                    toInclusive = DEFAULT_TO_INCLUSIVE;
                }
                E from  = (E)((SortedMapViewInfos.SubMapByFromKeyAndToKey)viewInfo).getFromKey();
                E to = (E)((SortedMapViewInfos.SubMapByFromKeyAndToKey)viewInfo).getToKey();
                this.acceptFrom(from, fromInclusive);
                this.acceptTo(to, toInclusive);
            }
        }
    ...
    private void acceptFrom(E from, boolean fromInclusive) {
        if (!this.hasFrom) {
            ...
            if (cmp > 0) {
                throw new IllegalArgumentException(LAZY_RESOURCE.get().parameterOutOfRange("from", from, this));
            }
            if (cmp == 0) {
                if (!this.fromInclusive) {
                    throw new IllegalArgumentException(LAZY_RESOURCE.get().parameterOutOfRange("from", from, this));
    ...
    private void acceptTo(E to, boolean toInclusive) {
        if (!this.hasTo) {
            ...
            if (cmp < 0) {
                throw new IllegalArgumentException(LAZY_RESOURCE.get().parameterOutOfRange("to", to, this));
            }
            if (cmp == 0) {
                if (!this.toInclusive) {
                    throw new IllegalArgumentException(LAZY_RESOURCE.get().parameterOutOfRange("to", to, this));
    ...
    private interface Resource {
        ...
        String parametersCanNotBeFalseSimultaneously(String hasFrom, String hasTo);
        
        String whenExpressionIsNullAndParameterIsTrue(String expression, String parameterName);
        
        String formGreaterThanTo(Object from, Object to);
        
        String parameterOutOfRange(String parameterName, Object argument, NavigableRange<?> range);
    }
}
