...
implements MANavigableMap<K, V> {
    ...
    public K firstKey() {
        ...
        throw new NoSuchElementException(LAZY_RESOURCE.get().noFirstKey(this.getClass()));
    }
    ...
    public K lastKey() {
        ...
        throw new NoSuchElementException(LAZY_RESOURCE.get().noLastKey(this.getClass()));
    }
    ...
    protected static abstract class AbstractSubMapImpl<K, V> extends AbstractMANavigableMap<K, V> implements MANavigableMapView<K, V> {
        ...
        protected void bubbleModifying(MapElementEvent<K, V> e) {
            AbstractMANavigableMap<K, V> parent = this.parentMap;
    ...
    implements MANavigableKeySetView<K, V> {
        ...
        protected final <T extends AbstractMANavigableMap<K, V>> T getParentMap() {
            ...
            if (parentMap == null) {
                throw new IllegalStateException(LAZY_RESOURCE.get().noParentMap(this.getClass()));
            }
            return (T)parentMap;
        }
    ...
    private interface Resource {
        ...
        String noFirstKey(Class<?> thisType);
        
        String noLastKey(Class<?> thisType);
        
        String noParentMap(Class<?> thisType);
    }
