...
implements NavigableBaseEntries<K, V> {
    ...
    protected final V putWithoutTriggerFlushing(
        ...
        Trigger<K, V> trigger = attachProcessor.getTrigger();
        ...
        if (comparator != null) {
            do {
                ...
                if (cmp < 0) {
                    ...
                    if (trigger != null) {
                        trigger.preChange(be, isKeyStrict ? key : be.key, value);
                    }
                    V oldV = be.value;
                    if (attachProcessor.beginExcute()) {
                        try {
        ...
        Throwable insertionThrowable = null;
        try {
            ...
            insertionThrowable = ex;
        }
        /*
         * RedBlackTree tree is special, we must execute the statement 
         * "attachProcessor.execute();" after the insertion.
         * Because the insertion depends on the local variable "parent"
         * that is assigned than the previous code. if the statement
         * is execute before the insertion, that local variable will be
         * be destroyed. 
         */
        attachProcessor.beginExcute();
        attachProcessor.endExecute(insertionThrowable);
        return null;
    }
    ...
    protected void putAllWithoutTriggerFlushing(
        ...
        if (this.root == null && m instanceof SortedMap<?, ?>) {
            Comparator<?> comparator = ((SortedMap<?, ?>) m).comparator();
            if (comparator == this.keyUnifiedComparator().comparator() ||
                    (comparator != null && comparator.equals(this.keyUnifiedComparator().comparator()))) {
                /*
                ...
                if (trigger != null) {
                    ...
                    for (int i = 0; i < mapSize; i++) {
                        ...
                        trigger.preAdd(entry.getKey(), entry.getValue());
                    }
                }
                if (attachProcessor.beginExcute()) {
                    try {
            ...
            if (attachProcessor.beginExcute()) {
                try {
                    for (Entry<? extends K, ? extends V> entry : m.entrySet()) {
                        this.putWithoutTriggerFlushing(
                                entry.getKey(), 
                                entry.getValue(), 
                                AbstractBaseEntriesImpl.<K, V>emptyAttachProcessor());
                    }
                    attachProcessor.endExecute(null);
                } catch (RuntimeException | Error ex) {
                    attachProcessor.endExecute(ex);
        ...
        History<K, V> history = trigger.getHistory(trigger.getLength());
        while (iterator.hasNext()) {
            ...
            if (be == null) {
                ...
                trigger.preChange(be, isKeyStrict ? k : be.key, v);
            }
        }
        if (attachProcessor.beginExcute()) {
            try {
                ...
                for(int i = 0; i < mapSize; i++) {
                    ...
                    if (be == null) {
                        this.put(
                                history.getNewKey(i), 
                                history.getNewValue(i), 
                                null,
                                null);
                    } else {
                        if (isKeyStrict) {
                            be.setRawKey(history.getNewKey(i), keyComparator);
                        }
                        be.value = history.getNewValue(i);
                    }
                }
                attachProcessor.endExecute(null);
            } catch (RuntimeException | Error ex) {
    ...
    protected boolean addAllWithoutTriggerFlushing(
        ...
        if (this.root == null && c instanceof SortedSet<?>) {
            Comparator<?> comparator = ((SortedSet<?>)c).comparator();
            if (comparator == this.keyUnifiedComparator().comparator() ||
                    (comparator != null && comparator.equals(this.keyUnifiedComparator().comparator()))) {
                /*
                 * Increase modCount at first to wreck the iterator if the
    ...
    private BaseEntryImpl<K, V> buildFromSorted(
        ...
        if (mid < hi) {
            BaseEntryImpl<K, V> right = this.buildFromSorted(level + 1, mid + 1,
                    hi, redLevel, iterator, stream, defaultVal);
            middle.right = right;
            right.parent = middle;
    ...
    private class SubTree 
        ...
        @Override
        public boolean isNonFairLockSupported() {
            return RedBlackTreeEntries.this.isNonFairLockSupported();
        }
        ...
        protected V put(K key, V value, Trigger<K, V> trigger, FrozenContextSuspending<K, V> suspending) {
            if (!this.range.contains(key)) {
                throw new IllegalArgumentException(LAZY_RESOURCE.get().elementOfKeyOutOfRange(this.range));
            }
    ...
    private interface Resource {
        ...
        String elementOfKeyOutOfRange(NavigableRange<?> range);
    }
}
