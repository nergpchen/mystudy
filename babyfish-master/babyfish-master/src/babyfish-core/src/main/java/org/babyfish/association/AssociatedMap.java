 ...
 * @author Tao Chen
 */
public abstract class AssociatedMap<O, K, V> 
extends AbstractWrapperMAMap<K, V>  
implements AssociatedEndpoint<O, V>, Serializable {

    private static final long serialVersionUID = 6772316108292945403L;
    ...
    protected AssociatedMap() {
        super(null);
    }
    
    @Override
    public final boolean isLoaded() {
        MAMap<K, V> base = this.getBase();
    ...
    private MapElementEventHandler<O, K, V> handler() {
        return new MapElementEventHandler<O, K, V>() {
            ...
            protected boolean isSuspended() {
                return AssociatedMap.this.suspended;
            }

            @Override
            protected void setSuspended(boolean suspended) {
                AssociatedMap.this.suspended = suspended;
            }

            @Override
            protected O getOwner() {
                return AssociatedMap.this.getOwner();
            }

            @Override
            protected boolean isLoadedObject(V opposite) {
                return AssociatedMap.this.isLoadedValue(opposite);
            }

            @Override
            protected boolean isAbandonableObject(V opposite) {
                return AssociatedMap.this.isAbandonableValue(opposite);
            }

            @Override
            protected void loadObject(V opposite) {
                AssociatedMap.this.loadValue(opposite);
            }

            @Override
            protected AssociatedEndpointType getOppositeEndpointType() {
                return AssociatedMap.this.getOppositeEndpointType();
            }

            @Override
            protected AssociatedEndpoint<V, O> getOppositeEndpoint(V opposite) {
                return AssociatedMap.this.getOppositeEndpoint(opposite);
            }
        };
    }
    ...
    protected static class RootData<K, V> extends AbstractWrapperMAMap.RootData<K, V> {
        ...
        private static final long serialVersionUID = 4213285351423544553L;
         ...
         * {@link MAMap#keyReplacementRule()} does not return {@link ReplacementRule#NEW_REFERENCE_WIN}
         */
        @Override
