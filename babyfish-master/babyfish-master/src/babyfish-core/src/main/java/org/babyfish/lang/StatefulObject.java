...
public abstract class StatefulObject {
    ...
    private void validateStream(Object stream, boolean in) {
        ...
        if (toppestSerializableClass == null) {
            throw new IllegalOperationException(LAZY_RESOURCE.get().ownerIsNotSerializable(
                    this.getClass(), 
                    String.format(
                            "%s(%s)", 
        ...
        if (this != obj) {
            throw new IllegalOperationException(
                    LAZY_RESOURCE.get().invalidIOTarget(
                            this.getClass(),
                            String.format(
                                    "%s(%s)", 
                                    in ? "readState" : "writeState", 
        ...
        if (toppestSerializableClass != desc.forClass()) {
            throw new IllegalOperationException(
                    LAZY_RESOURCE.get().invalidIOOwner(
                            this.getClass(), 
                            String.format(
                                    "%s(%s)", 
                                    in ? "readState" : "writeState", 
            ...
            if (toppestSerializableClass == null) {
                ...
                for (Method method : clazz.getDeclaredMethods()) {
                    if (method.getName().equals("onWriteState") &&
                        ...
                        throw new IllegalProgramException(LAZY_RESOURCE.get().invalidOverriding(method, method.getDeclaringClass()));
