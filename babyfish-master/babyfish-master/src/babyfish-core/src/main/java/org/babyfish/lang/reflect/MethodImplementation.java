...
public class MethodImplementation {
    ...
    private static class Context {
        ...
        void add0(ClassInfo<?> classInfo, Set<ClassInfo<?>> interfaces, Map<Class<?>, ClassInfo<?>> map) {
            ...
            if (classInfo.isInterface()) {
                ...
                if (this.clazz != null) {
                    throw new IllegalArgumentException(LAZY_RESOURCE.get().twoManyClasses(this.clazz, classInfo));
                }
        ...
        static void add0(ClassInfo<?> classInfo, Map<Class<?>, ClassInfo<?>> map) {
            ...
            if (ci != null && !ci.equals(classInfo)) {
                throw new IllegalArgumentException(
                        LAZY_RESOURCE.get().duplicatedTypesWithDifferentTypeArguments(ci, classInfo)
                );
            }
    ...
    private interface Resource {
        ...
        String twoManyClasses(ClassInfo<?> classInfo1, ClassInfo<?> classInfo2);
        
        String duplicatedTypesWithDifferentTypeArguments(ClassInfo<?> classInfo1, ClassInfo<?> classInfo2);
    }
}
