...
public abstract class ModifierSet implements Set<Modifier>, Serializable, Cloneable {
        ...
        for (Conflict conflict : conflicts) {
            if (conflict.modifier1 == newModifier && this.contains(conflict.modifier2)) {
                throw new IllegalArgumentException(
                        LAZY_RESOURCE.get().conflictModifier(ModifierSet.class, newModifier, conflict.modifier2)
                );
            }
        }
    }
    ...
    private class IteratorImpl implements Iterator<Modifier> {
        ...
        public Modifier next() {
            if (this.expectModCount != ModifierSet.this.modCount) {
                throw new ConcurrentModificationException(LAZY_RESOURCE.get().concurrentModifcation());
            }
            if (this.preparedOrdinal >= MAX_SIZE) {
                throw new NoSuchElementException(LAZY_RESOURCE.get().noSuchElement());
            }
        ...
        public void remove() {
            ...
            if (this.lastRetOrdinal == -1) {
                throw new IllegalStateException(LAZY_RESOURCE.get().removeNoExtractedElement());
            }
    ...
    private static class ForType extends ModifierSet {
        ...
        void validateModifier(Modifier e) {
            if (!e.forTypes) {
                throw new IllegalArgumentException(LAZY_RESOURCE.get().invalidTypeModifier(e));
            }
    ...
    private static class ForField extends ModifierSet {
        ...
        void validateModifier(Modifier e) {
            if (!e.forFields) {
                throw new IllegalArgumentException(LAZY_RESOURCE.get().invalidFieldModifier(e));
            }
            this.validateNoConflicts(e, CONFLICTS);
    ...
    private static class ForMethod extends ModifierSet {
        ...
        void validateModifier(Modifier e) {
            if (!e.forMethods) {
                throw new IllegalArgumentException(LAZY_RESOURCE.get().invalidMethodModifier(e));
            }
            this.validateNoConflicts(e, CONFLICTS);
    ...
    private interface Resource {
        ...
        String conflictModifier(
                Class<ModifierSet> modifierSetType,
                Modifier newModifier,
                Modifier existingModifier);
        
        String concurrentModifcation();
        
        String noSuchElement();
        
        String removeNoExtractedElement();
        
        String invalidTypeModifier(Modifier modifier);
        
        String invalidFieldModifier(Modifier modifier);
        
        String invalidMethodModifier(Modifier modifier);
    }
