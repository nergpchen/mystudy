...
public abstract class Singleton {
        ...
        try {
            if (!enableConstructor) {
                throw new IllegalProgramException(
                        LAZY_RESOURCE.get().illegalConstructorInvocation(
                                this.getClass(), 
                                Singleton.class
                        )
                );
    ...
    private static Object createInstance(Class<? extends Singleton> clazz) {
        ...
        if (constructor == null) {
            throw new IllegalProgramException(LAZY_RESOURCE.get().noDefaultConstructor(clazz));
        }
        if (Modifier.isPublic(constructor.getModifiers())) {
            throw new IllegalProgramException(LAZY_RESOURCE.get().defaultConstructorMustNotBePublic(clazz));
        }
        for (Class<? extends Throwable> exceptionType : (Class[])constructor.getExceptionTypes()) {
            if (!UncheckedException.isRuntimeExceptionOrError(exceptionType)) {
                throw new IllegalProgramException(
                        LAZY_RESOURCE.get().defaultConstructorThrowsCheckedException(
                                clazz, 
                                (Class)exceptionType
                        )
                );
    ...
    private interface Resource {
        ...
        String illegalConstructorInvocation(
        ...
        String defaultConstructorMustNotBePublic(
                Class<? extends Singleton> implType);
        
        String defaultConstructorThrowsCheckedException(
                Class<? extends Singleton> implType,
