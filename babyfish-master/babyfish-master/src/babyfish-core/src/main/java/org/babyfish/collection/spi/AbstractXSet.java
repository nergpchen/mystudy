...
public abstract class AbstractXSet<E> extends StatefulObject implements XSet<E> {
        ...
        if (!(this instanceof View)) {
            if (this instanceof XMap.XKeySetView) {
                Arguments.mustNotBeInstanceOfValueWhen(
                        LAZY_RESOURCE.get().whenThisIsInstanceofXKeySetView(XKeySetView.class), 
                        "baseEntries", 
                        baseEntries, 
                        TransientValueEntries.class);
            } else {
                Arguments.mustBeInstanceOfValueWhen(
                        LAZY_RESOURCE.get().whenThisIsNotInstanceofXKeySetView(XKeySetView.class),
                        "baseEntries", 
                        baseEntries, 
                        TransientValueEntries.class);
            }
    ...
    public boolean contains(Object o, ElementMatcher<? super E> matcher) {
        return this.baseEntries.containsKey(o, matcher);
    }
    ...
    public boolean containsAll(Collection<?> c, ElementMatcher<? super E> matcher) {
        ...
        for (Object e : c) {
            if (!baseEntries.containsKey(e, matcher)) {
                return false;
            }
        }
        return true;
    ...
    public XIterator<E> iterator() {
        ...
        final boolean nonFairLockSupported = this.baseEntries.isNonFairLockSupported();
    ...
    private interface Resource {
        ...
        String whenThisIsInstanceofXKeySetView(Class<XKeySetView> keySetViewType);

        @SuppressWarnings("rawtypes")
        String whenThisIsNotInstanceofXKeySetView(Class<XKeySetView> keySetViewType);
    }
}
