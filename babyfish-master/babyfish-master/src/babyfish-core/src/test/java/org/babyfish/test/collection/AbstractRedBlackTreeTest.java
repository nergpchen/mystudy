...
public abstract class AbstractRedBlackTreeTest {
    ...
    private void testWithJDKTreeMap(boolean descending) {
        NavigableMap<Integer, Object> jdkAscTreeMap = new TreeMap<Integer, Object>();
        ...
        if (descending) {
            jdkTreeMap = jdkTreeMap.descendingMap();
            xTreeMap = xTreeMap.descendingMap();
        }
        ...
        /*
         * descendingMAMap will cause the comparator of this object and the comparator
         * of parameter map is different when the XNavibaleMap.putAll(m) is executed.
         * If the XNavigableMap is instance of MANavigableMap, it will removed all
         * the duplicated keys of the m by recreate m so that the xmap is different with map.
         */
        boolean strictCompare = !(descending && xAscTreeMap instanceof MANavigableMap<?, ?>);
    ...
    private static void compareTreeMap(NavigableMap<?, ?> jdkTreeMap, XNavigableMap<?, ?> xTreeMap) {
        ...
        Object baseTreeEntries = getFieldValue(xTreeMap, "baseEntries");
        Object redBlackTree;
        if (baseTreeEntries instanceof RedBlackTreeEntries<?, ?>) {
            redBlackTree = baseTreeEntries;
        } else {
            redBlackTree = getFieldValue(baseTreeEntries, "this$0"); 
        }
