...
public class FrozenContextTest {
    ...
    @Test
    public void testByOrderedSetWithConflictVoter() {
        XOrderedMap<StringRef, StringRef> map = new LinkedHashMap<>(
                StringRef.VFE, 
                UnifiedComparator.<StringRef>empty());
        map.addConflictVoter(new MapConflictVoter<StringRef, StringRef>() {
            @Override
            public void vote(MapConflictVoterArgs<StringRef, StringRef> args) {
                MapReader<StringRef, StringRef> reader = args.reader();
                while (reader.read()) {
                    if (args.newValue().getValue().equalsIgnoreCase(reader.value().getValue())) {
                        args.conflictFound(reader.key());
                    }
                }
            }
        });
        StringRef one = new StringRef("one");
        StringRef two = new StringRef("two");
        StringRef three = new StringRef("three");
        map.put(one, one);
        map.put(two, two);
        map.put(three, three);
        
        assertStringRefMap(map, "one", "two", "three");
        one.setValue("TWO");
        assertStringRefMap(map, "TWO", "three");
    }
    ...
    public void testByMANavigableMap() {
        ...
        map.addMapElementListener(
                new MapElementAdapter<Element, Object>() {
                    @Override
    ...
    private static void assertStringRefMap(Map<StringRef, StringRef> map, String ... values) {
        Assert.assertEquals(values.length, map.size());
        int index = 0;
        for (Map.Entry<StringRef, StringRef> entry : map.entrySet()) {
